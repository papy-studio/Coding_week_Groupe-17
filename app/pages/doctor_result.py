...
# Cherche le meilleur modèle disponible dans outputs/
MODEL_PRIORITY = [
...
.section-card {
    ...
    border-radius: 16px; padding: 28px 32px; margin-bottom: 20px; position: relative; overflow: hidden;
}
...
.save-btn [data-testid="stButton"] > button { background: linear-gradient(135deg, #52B788, #3d9e70) !important; color: #FFFFFF !important; border: none !important; border-radius: 10px !important; padding: 13px 0 !important; font-family: 'DM Sans', sans-serif !important; font-size: 14.5px !important; font-weight: 500 !important; box-shadow: 0 4px 16px rgba(82,183,136,0.30) !important; }
.save-btn [data-testid="stButton"] > button:hover { transform: translateY(-1px) !important; }
.reco-btn [data-testid="stButton"] > button { background: linear-gradient(135deg, #1D6996, #155a7a) !important; color: #FFFFFF !important; border: none !important; border-radius: 10px !important; padding: 13px 0 !important; font-family: 'DM Sans', sans-serif !important; font-size: 14.5px !important; font-weight: 500 !important; box-shadow: 0 4px 16px rgba(29,105,150,0.30) !important; }
.new-btn [data-testid="stButton"] > button { background: transparent !important; color: #4A7A9A !important; border: 1px solid rgba(29,105,150,0.30) !important; border-radius: 10px !important; padding: 12px 0 !important; font-family: 'DM Sans', sans-serif !important; font-size: 14px !important; box-shadow: none !important; }
</style>
...
# ── Load model (LightGBM en priorité) ─────────────────────────────────────────
@st.cache_resource
def load_best_model():
...
# ── Model check ────────────────────────────────────────────────────────────────
if model is None:
    st.error("⚠️ Aucun modèle trouvé dans `outputs/` ou `models/`.")
    st.info("Vérifiez que `outputs/lightgbm.pkl` existe.")
    st.stop()

# Badge modèle utilisé
st.markdown(f'<div class="model-badge">🤖 Modèle utilisé : {model_name}</div>', unsafe_allow_html=True)

# ── Prediction ─────────────────────────────────────────────────────────────────
raw_pred = model.predict(df_input)
probas   = model.predict_proba(df_input)[0]

# Normaliser la prédiction → int
raw_val = np.array(raw_pred).flatten()[0]
try:    pred_int = int(raw_val)
except: pred_int = 0

pred_label    = OBESITY_LABELS.get(pred_int, CLASSES_ORDER[pred_int] if pred_int < len(CLASSES_ORDER) else "Normal_Weight")
...
with col_res:
    ...
    <div class="result-card" style="background:{bg}; --accent-color:{accent};">
        ...
        <div style="margin-bottom:12px;">
            <span style="background:{accent}22; border:1px solid {accent}55; border-radius:20px;
                         padding:4px 14px; font-size:12px; color:{accent};">
...
with col_proba:
    ...
    for i in sorted_idx:
        # Récupérer le label de la classe i
...
# ══════════════════════════════════════════════════════════════════════════════
# BLOC 2 — SHAP (logique du notebook espace_medecin)
# ══════════════════════════════════════════════════════════════════════════════
...
try:
    ...
    # S'assurer que l'index est un entier Python natif
    class_idx = int(pred_int)

    if isinstance(shap_values, np.ndarray) and shap_values.ndim == 3:
        # Nouvelle version LightGBM → shape (1, 16, 7)
        sv = shap_values[0, :, class_idx]
    elif isinstance(shap_values, list):
        # Ancienne version → liste de 7 arrays shape (1, 16)
    ...
    with st.expander("📈 Summary plot global"):
        ...
        # LightGBM → shap_values[pred_int] ou shap_values[:,:,pred_int]
        if isinstance(shap_values, np.ndarray) and shap_values.ndim == 3:
